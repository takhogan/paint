...
public class PaintController {
    ...
    public ModelAndView CurrencyMarketServlet(HttpServletRequest req,
        ...
        List<Trade> tlist = transactionService.listTrades();
        req.setAttribute("otherlist", tlist);
        String[] varNames = Order.getVarNames();
        req.setAttribute("varNames", varNames);
        req.setAttribute("nVars",varNames.length);
        return mav;
    }
    ...
    public ModelAndView MarketServlet(HttpServletRequest req,
        ...
        mav.addObject("nVars", req.getAttribute("nVars"));
    ...
    public ModelAndView BondMarketServlet(HttpServletRequest req,
        ...
        ModelAndView mav = new ModelAndView("forward:/market");
        mav.addObject("user",user);
        LinkedList<Bond> inactiveblist = new LinkedList<>(transactionService.listBonds());
        LinkedList<Bond> activeblist = new LinkedList<>();
        for(Bond b : inactiveblist){
            int active = b.getActive();
            if(active != 0){
                inactiveblist.remove(b);
                activeblist.add(b);
            }
        }
        req.setAttribute("mlist", inactiveblist);
        mav.addObject("mlist", inactiveblist);
        mav.addObject("activeblist",activeblist);
        return mav;
    }
    //INCOMPLETE
    @RequestMapping(value = "/futures")
    public ModelAndView FuturesMarketServlet(@CookieValue("username") String user){
        ModelAndView mav = new ModelAndView("forward:/market");
        mav.addObject("user", user);
        return mav;
    }
    ...
    public ModelAndView BondExecutionServlet(HttpServletRequest req,
        ...
        for(Integer b : bid) {
            ...
            //if lender (someone is offering to borrow)
                ...
                //wallets
                Wallet receiver = transactionService.getBigWallet(bond.getBorrower(), c_name);
    ...
    public ModelAndView FuturesGenerationServlet(HttpServletRequest req,
                                                 ...
                                                 @RequestParam("expiration_units") Integer[] expiration_units){
        ModelAndView mav = new ModelAndView("forward:/execute");
        mav.addObject("user", user);
        LinkedList<FinanceObject> flist = new LinkedList<>();
        ...
        for(int i = 0; i < wantname.length; i++) {
            //check validity
            Wallet originator = transactionService.getBigWallet(user, givename[i]);
            ...
            String status = FuturesContract.verifyMargin(originator.getBalance(), wantamount[i], transactionService.getLatestRate(wname, gname));
            ...
            int expiry = expiration_value[i]*expiration_units[i]*60; //stored as seconds
            if(expiry < 0){
                status = "invalid expiration period";
            }

            //add to database
            FuturesContract fc = new FuturesContract(cr_type, user, wname, wam, gname, gam, expiry);
            if(status == null) {
    ...
    public ModelAndView BondGenerationServlet(HttpServletRequest req,
                                              ...
                                              @RequestParam(value = "freq_mult", required = false) Integer freq_mult,
        ...
        if(!(c_amount > 0 && c_amount < 1000000)){
            status = "invalid c_amount";
        }
        int expiry = exp_val*exp_units*60; //stored as seconds
        if(!(expiry > 0)){
            status = "invalid expiration period";
        }
        //create bond object
        Bond b = new Bond(user, cr_type, c_name, c_amount, initial_margin, freq, loan_type, i_amount, expiry);
        if(status == null){
            double sum = transactionService.sumUserBalances(user, c_name);
            //if lender
            if(cr_type == 1){
                //check that lender has loan amount available
                if(!(sum >= c_amount)){
                    status = "insufficient funds: need at least " + c_amount + " c_name";
                }
            }
            //if borrower (nothing)
        }
    ...
    public void manage_futures(){
        ...
        for(FuturesContract fc : contrlist){
            long expiration = fc.getExpiration();
            ZonedDateTime orig = fc.getOrigination();
            ZonedDateTime expiry = orig.plusSeconds(expiration);
            ZonedDateTime now = ZonedDateTime.now();

            String buyer = fc.getBuyer_name();
            String buycurrency = fc.getWantname();
            Wallet buyerwallet = transactionService.getBigWallet(buyer, buycurrency);
            double buyamount = fc.getWantamount();

            String seller = fc.getSeller_name();
            String sellcurrency = fc.getGivename();
            Wallet sellerwallet = transactionService.getBigWallet(seller, sellcurrency);
            double sellamount = fc.getGiveamount();
